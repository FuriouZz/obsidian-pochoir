...
dntShim.Deno.test("Print tag", async () => {
    await test({
        template: `
    {{ "Hello world" }}
    `,
...
dntShim.Deno.test("Print tag with variable", async () => {
    await test({
        template: `
    {{ it.message }}
    `,
...
dntShim.Deno.test("Print tag with condition", async () => {
    await test({
        template: `
    {{ message || "Hello world" }}
    `,
...
dntShim.Deno.test("Print tag with filters", async () => {
    await test({
        template: `
    {{ message |> toUpperCase }}
    `,
    ...
    `,
        ...
        data: { message: "Hello World" },
        init(env) {
            env.filters.slugify = (value) => value.toLowerCase().replace(/\s/g, "-");
        },
    });
});
dntShim.Deno.test("Print trim", async () => {
    await test({
...
dntShim.Deno.test("Print trim with filter", async () => {
    await test({
...
dntShim.Deno.test({
    ...
    fn: async () => {
        ...
        const expected = JSON.stringify(JSON.parse(dntShim.Deno.readTextFileSync(new URL(url))));
        await test({
