...
dntShim.Deno.test("Include tag", async () => {
    await test({
        template: `
...
dntShim.Deno.test("Include tag (autoescaped enabled)", async () => {
    await test({
        options: {
            autoescape: true,
        },
...
dntShim.Deno.test("Include tag (with filters)", async () => {
    await test({
        template: `
...
dntShim.Deno.test("Include tag (with data)", async () => {
    await test({
        template: `
    {{ include "/my-file.tmpl" }}
    `,
...
dntShim.Deno.test("Include tag (with custom data)", async () => {
    await test({
        template: `
    ...
    `,
        ...
        includes: {
            "/sub/my-file.tmpl": "{{ include './other-file.tmpl' { name: `${name} Óscar`} }}",
...
dntShim.Deno.test("Include tag (with custom data and filters)", async () => {
    await test({
        template: `
...
dntShim.Deno.test("Include tag (with front matter)", async () => {
    await test({
        template: `
...
dntShim.Deno.test("Include tag dynamically", async () => {
    await test({
        template: `
    {{ include file }}
    `,
...
dntShim.Deno.test("Include tag with object shorthand syntax", async () => {
    await test({
        template: `
