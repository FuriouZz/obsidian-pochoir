...
dntShim.Deno.test("Set tag", async () => {
    await test({
        template: `
...
dntShim.Deno.test("Set tag with complex", async () => {
    await test({
        template: `
...
dntShim.Deno.test("Set tag with filters", async () => {
    await test({
        template: `
    {{ set message = ["Hello", "world"] |> join(" ") }}
    {{ message }}
    ...
    `,
        ...
        filters: {
            filt: (arr) => arr.map((a) => a.toUpperCase()).join(" "),
        },
    });
});
dntShim.Deno.test("Set tag with includes", async () => {
    await test({
        template: `
    {{ set name = "World" }}
...
dntShim.Deno.test("Set tag in a loop", async () => {
    await test({
        template: `
    {{- for item of items -}}
    <div>
    ...
    {{- /for -}}
    `,
        expected: "<div><h1>Name 1</h1><p>Description 1</p></div><div><h1>Name 2</h1><p>Description 2</p></div><div><h1>Name 3</h1><p>Description 3</p></div>",
        data: {
            items: [
